...
func webThreeWithLogin(c *http.Client, login, password string) (string, error) {
	// replace +44 to 0
	...
	var lt string
	{
		...
		if err != nil {
			return "", errors.Wrap(err, "login get cookies NewDocumentFromReader")
		}
		...
		if !ok {
			...
			if !ok {
				return "", errors.Wrap(err, "could not extract redirect url from response")
			}
		}
	...
	if authUrl == "" {
		...
		if err != nil {
			return "", errors.Wrap(err, "login Get")
		}
		defer resp.Body.Close()

		doc, err := goquery.NewDocumentFromReader(resp.Body)
		if err != nil {
			return "", errors.Wrap(err, "login post NewDocumentFromReader")
		}

		var ok bool
		authUrl, ok = doc.SetHtml(doc.Find("div.P00_id > noscript").Text()).Find("p > a").First().Attr("href")
		if !ok {
			return "", errors.Errorf("could not extract redirect url from response")
		}
	}
